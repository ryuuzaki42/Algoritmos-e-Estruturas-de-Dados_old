...
void insereUltimo(int x, celula *p) /* Insere no final da lista*/
{
	...
	nova->ant = q; // nova celula recebe conteúdo de entrada
	nova->prox = NULL; // Não "aponta" para outra celula
}


void buscaEremove (int y, celula *p)
{
   celula *w, *q; // servem como auixiliares para não perder algum nó durante a operação
   w = p; 
   q = p->prox;
